...
The public Python application exposes `GET /health`, which returns only `ok`. The management ASGI app runs on the internal management port (`8081` by default), deliberately not published by Compose: `GET /health/liveness` and `GET /health/readiness` are available from inside the application container or a private monitoring network. Do not expose the management port through a public reverse proxy.
...
Collect container stdout/stderr and the metrics endpoint privately. Alert on startup or Alembic failures, readiness failures, webhook verification failures, rejected senders, AI-provider and Open Food Facts failures, duplicate-update rates, report delivery failures, outbox retry age, and database health. Do not log tokens, raw media, or full private messages.
