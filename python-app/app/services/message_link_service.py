...
from app.db.constraints import LINK_CODE_TTL
...
async def issue(session: AsyncSession, user: FoodUser) -> str:
    while True:
        ...
        if not await frontend_link_code_repo.exists(session, code):
            break
    session.add(FrontendLinkCode(code=code, user_id=user.id, expires_at=datetime.now(UTC) + LINK_CODE_TTL))
    await session.flush()
    return code
...
async def redeem(session: AsyncSession, code: str, provider: str, external_user_id: str, conversation_id: str) -> FoodUser:
    ...
    link = await frontend_link_code_repo.find_by_code(session, code, for_update=True)
