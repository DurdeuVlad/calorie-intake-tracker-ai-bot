...
plain session) by executing the exact same shared SQL against the
already-migrated test database -- safe to re-run since the WHERE clause only
matches rows still onboarding_completed = false."""
...
async def test_backfill_completes_an_engaged_user_stuck_on_timezone():
    ...
    async with session_scope() as session:
        ...
        await session.execute(text(STUCK_ONBOARDING_BACKFILL_SQL))
        await session.commit()
...
@pytest.mark.asyncio
async def test_backfill_ignores_differently_cased_or_padded_start_replays():
    """conversation_memory stores the raw message verbatim (only length-
    truncated, see conversation_memory_service.py's _truncate) -- a user who
    only ever retyped /start with different casing or stray whitespace must
    not be mistaken for someone who actually replied to the bot."""
    async with session_scope() as session:
        user = await get_or_create_by_telegram_user_id(session, 9004, "Tester", "Europe/Bucharest")
        now = datetime.now(UTC)
        session.add(ConversationMemory(user_id=user.id, role="user", content="/start", created_at=now))
        session.add(ConversationMemory(user_id=user.id, role="assistant", content="Welcome...", created_at=now))
        session.add(ConversationMemory(user_id=user.id, role="user", content="/Start", created_at=now))
