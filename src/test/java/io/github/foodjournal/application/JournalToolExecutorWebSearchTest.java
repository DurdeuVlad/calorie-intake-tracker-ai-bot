...
class JournalToolExecutorWebSearchTest {
  ...
  private JournalToolExecutor executor(SearxngClient searxng,BrowserlessClient browserless){JournalToolExecutor executor=new JournalToolExecutor(new ObjectMapper(),null,null,null,null,null,null,null,null,null,null,null,null,searxng,browserless);executor.redirectResolver=url->url;return executor;}
  ...
  @Test void fetchWebPageRejectsUrlsWhoseRedirectChainLeadsInternal(){BrowserlessClient browserless=mock(BrowserlessClient.class);JournalToolExecutor executor=executor(null,browserless);executor.redirectResolver=url->null;AgentToolResult result=execute(executor,"fetch_web_page","{\"url\":\"https://tacobell.com/menu/burrito\"}");assertThat(result.code()).isEqualTo("VALIDATION_ERROR");verifyNoInteractions(browserless);}
  @Test void fetchWebPagePassesTheFullyResolvedUrlToBrowserless(){BrowserlessClient browserless=mock(BrowserlessClient.class);JournalToolExecutor executor=executor(null,browserless);executor.redirectResolver=url->"https://tacobell.com/menu/burrito-supreme";when(browserless.fetchText("https://tacobell.com/menu/burrito-supreme")).thenReturn(Optional.of("710 calories"));AgentToolResult result=execute(executor,"fetch_web_page","{\"url\":\"https://tacobell.com/menu/burrito\"}");assertThat(result.ok()).isTrue();assertThat(result.data().get("url")).isEqualTo("https://tacobell.com/menu/burrito-supreme");verify(browserless).fetchText("https://tacobell.com/menu/burrito-supreme");}
}
