...
class ConversationalJournalApplicationServiceTest {
 ...
 @Test void answersRomanianTodayTotalWithoutCallingAi(){FoodEntry entry=new FoodEntry(user,"lapte",Instant.now(),297,"manual","high");when(entries.findByUserAndEatenAtBetweenOrderByEatenAtAsc(eq(user),any(),any())).thenReturn(List.of(entry));assertThat(service.handle(1L,1L,"Vlad","total azi?")).contains("Total azi: 297 kcal");verifyNoInteractions(interpreter);}
