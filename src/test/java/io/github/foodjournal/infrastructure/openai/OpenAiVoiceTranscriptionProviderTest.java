...
class OpenAiVoiceTranscriberTest {
  ...
  @Test void legacyTelegramPathDownloadsOnceAndErasesTransientBytes(){
    byte[] bytes={4,5,6};RestClient.Builder builder=RestClient.builder().baseUrl("https://api.openai.com/v1");MockRestServiceServer server=MockRestServiceServer.bindTo(builder).build();
    server.expect(once(),requestTo("https://api.openai.com/v1/audio/transcriptions")).andRespond(withSuccess("{\"text\":\"150 g chicken\"}",MediaType.APPLICATION_JSON));
    String text=new OpenAiVoiceTranscriber(builder.build(),props(),id->new TransientVoicePayload(bytes)).transcribe("voice-42","audio/ogg");
    assertThat(text).isEqualTo("150 g chicken");assertThat(bytes).containsOnly((byte)0);server.verify();
  }
