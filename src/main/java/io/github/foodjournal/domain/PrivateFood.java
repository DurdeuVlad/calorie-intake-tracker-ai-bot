...
@Entity @Table(name="private_foods",uniqueConstraints=@UniqueConstraint(columnNames={"user_id","name"})) public class PrivateFood {
 @Id @GeneratedValue(strategy=GenerationType.IDENTITY) private Long id; @ManyToOne(optional=false) @JoinColumn(name="user_id") private FoodUser user; @Column(nullable=false) private String name; @Column(name="calories_per_100g") private Integer caloriesPer100g; @Column(name="protein_per_100g") private BigDecimal proteinPer100g; @Column(name="carbs_per_100g") private BigDecimal carbsPer100g; @Column(name="fat_per_100g") private BigDecimal fatPer100g; @Column(nullable=false) private Instant createdAt=Instant.now();
