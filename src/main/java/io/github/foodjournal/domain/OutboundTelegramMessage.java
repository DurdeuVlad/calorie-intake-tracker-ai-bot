...
public class OutboundTelegramMessage {
  ...
  public void markSent(){status=Status.SENT;sentAt=Instant.now();}
  public void scheduleRetry(){attempts++;nextAttemptAt=Instant.now().plusSeconds(Math.min(300, 1L << Math.min(attempts, 8)));}
}
