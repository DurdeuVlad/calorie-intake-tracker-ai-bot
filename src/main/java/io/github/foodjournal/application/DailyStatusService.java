...
import io.github.foodjournal.domain.*; import io.github.foodjournal.repository.*; import io.github.foodjournal.messaging.MessagingExecutionContext; import java.time.*; import java.util.*; import org.springframework.stereotype.Service; import org.springframework.transaction.annotation.Transactional;
@Service public class DailyStatusService {
 ...
 @Transactional public void rememberMessage(Long id,java.util.UUID token,Long messageId){statuses.findById(id).ifPresent(status->status.rememberMessage(token,messageId));}
