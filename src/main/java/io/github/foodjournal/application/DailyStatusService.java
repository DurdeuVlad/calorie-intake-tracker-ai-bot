...
@Service public class DailyStatusService {
 ...
 @Transactional public void markDelivered(Long id,long version,Long messageId){statuses.findById(id).ifPresent(status->status.delivered(version,messageId));}
 @Transactional public void clearMessage(Long id){statuses.findById(id).ifPresent(PinnedDailyStatus::clearTelegramMessage);}
}
