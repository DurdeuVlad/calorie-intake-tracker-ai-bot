...
public class ConversationMemoryService {
 ...
 @Transactional public void recordTurn(FoodUser user,String userMessage,String assistantMessage){FoodUser locked=users.lockById(user.getId()).orElseThrow();memory.save(new ConversationMemory(locked,"user",truncate(userMessage)));memory.save(new ConversationMemory(locked,"assistant",truncate(assistantMessage)));List<ConversationMemory> rows=memory.findByUserOrderByCreatedAtDescIdDesc(locked);if(rows.size()>MAX_MESSAGES)memory.deleteAll(rows.subList(MAX_MESSAGES,rows.size()));}
