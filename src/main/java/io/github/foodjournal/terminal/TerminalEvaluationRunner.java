...
public class TerminalEvaluationRunner {
  ...
  private void cleanUserState(){ db.update("delete from outbound_telegram_messages where chat_id = ?",terminal.userId()); db.update("delete from food_users where telegram_user_id = ?",terminal.userId()); }
