...
public interface OutboundTelegramMessageRepository extends JpaRepository<OutboundTelegramMessage,Long>{
  @Query(value="select * from outbound_telegram_messages where (status = 'PENDING' and next_attempt_at <= current_timestamp) or (status = 'IN_PROGRESS' and lease_expires_at <= current_timestamp) order by id for update skip locked limit 20", nativeQuery=true)
