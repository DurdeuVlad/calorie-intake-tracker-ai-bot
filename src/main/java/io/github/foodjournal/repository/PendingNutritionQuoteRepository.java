...
public interface PendingNutritionQuoteRepository extends JpaRepository<PendingNutritionQuote, UUID> {
  Optional<PendingNutritionQuote> findFirstByUserAndTypeAndExpiresAtAfterOrderByCreatedAtDesc(FoodUser user, PendingNutritionQuote.Type type, Instant now);
  List<PendingNutritionQuote> findByUserAndBatchIdAndExpiresAtAfterOrderByCreatedAtAsc(FoodUser user, UUID batchId, Instant now);
