...
public class TelegramInboxWorker {
 ...
 @Scheduled(fixedDelayString="${food-journal.inbox-delay-ms:500}") public void dispatch(){TelegramInboxClaimService.Claim claim=claims.claim();if(claim==null)return;try{updates.processQueued(json.readValue(claim.payload(),TelegramUpdate.class),claim.updateId(),claim.token());}catch(Exception failure){claims.retry(claim.updateId(),claim.token(),category(failure));}}
