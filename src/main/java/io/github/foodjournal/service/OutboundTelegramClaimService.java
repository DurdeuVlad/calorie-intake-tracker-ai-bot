...
@Service public class OutboundTelegramClaimService {
 public record Delivery(Long id,long chatId,String text,Long sourceUpdateId,UUID leaseToken) {}
 ...
 @Transactional public List<Delivery> claimBatch(){return messages.lockReadyForDelivery().stream().map(message->{message.claim();return new Delivery(message.getId(),message.getChatId(),message.getText(),message.getSourceUpdateId(),message.getLeaseToken());}).toList();}
