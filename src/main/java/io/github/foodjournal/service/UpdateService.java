...
@Service public class UpdateService {
 ...
 private void process(TelegramUpdate update,boolean claim,Long sourceUpdateId){
   ...
   String text=msg.text(); if((text==null||text.isBlank())&&msg.voice()!=null){try{text=voice.transcribe(msg.voice().file_id(),msg.voice().mime_type());}catch(Exception failure){log.warn("voice_transcription_failed update_id={} category={}",update.update_id(),failure instanceof MediaProcessingException mediaFailure?mediaFailure.category():"UNKNOWN");reply(msg.chat().id(),voiceFailureReply(msg.from()),sourceUpdateId);return;}}
