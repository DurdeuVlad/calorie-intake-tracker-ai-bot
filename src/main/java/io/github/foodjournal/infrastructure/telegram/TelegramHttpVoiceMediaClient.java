...
public class TelegramHttpVoiceMediaClient implements TelegramVoiceMediaClient {
  ...
  @Override public TransientVoicePayload download(String telegramFileId){
    ...
    try{Map<?,?> response=telegram.get().uri(uri->uri.path("/bot/{token}/getFile").queryParam("file_id",telegramFileId).build(properties.telegramToken())).retrieve().body(Map.class);Object result=response==null?null:response.get("result");if(!(result instanceof Map<?,?> file)||!(file.get("file_path") instanceof String path)||path.isBlank())throw new MediaProcessingException(MediaProcessingException.Category.TELEGRAM_DOWNLOAD,"Telegram media is unavailable");String rawPath=path.startsWith("/")?path.substring(1):path;byte[] bytes=telegram.get().uri(java.net.URI.create("https://api.telegram.org/file/bot"+properties.telegramToken()+"/"+rawPath)).retrieve().body(byte[].class);if(bytes==null||bytes.length==0)throw new MediaProcessingException(MediaProcessingException.Category.TELEGRAM_DOWNLOAD,"Telegram media is unavailable");return new TransientVoicePayload(bytes);}catch(MediaProcessingException expected){throw expected;}catch(RestClientResponseException|ResourceAccessException failure){throw new MediaProcessingException(MediaProcessingException.Category.TELEGRAM_DOWNLOAD,"Telegram media download failed",failure);}
  }
}
